...
* How to Test :
1) Geozone Creation: Click "Create," draw a polygon on the map (minimum 3 vertices), and assign it a category (e.g., War Risk).

2) Livewire Filtering: On the list page, type a name in the search bar or change the category filter to see the list update dynamically without a page refresh.

3) Spatial Integrity: Edit an existing zone and move the vertices. The system uses ST_IsValid to ensure the geometry remains valid before saving to PostGIS.
